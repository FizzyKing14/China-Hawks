...
CHARGES = [
    ...
    ("二·人身", "故意袭击罪", "重罪", 36, 15000,
     ["故意袭击", "袭击", "殴打", "打人", "动手打", "拳打脚踢", "暴力伤人", "揍", "打伤",
      "打了一顿", "围殴", "动手", "打架", "击伤", "打伤了", "把人打伤", "击打", "打成伤",
      "揍伤", "殴伤", "拳打", "脚踢", "暴打", "痛打"],
     "蓄意暴力致他人身体受伤"),
    ...
    ("三·财产", "抢劫罪", "重罪", 60, 20000,
     ["抢劫", "持械抢劫", "持枪抢劫", "抢夺财物", "武力抢", "威胁抢", "打劫", "抢东西", "抢钱", "洗劫", "抢了",
      "去抢", "抢便利店", "抢商店", "抢银行", "抢劫便利店", "抢劫商店", "抢劫银行", "抢包", "抢劫路人",
      "持枪打劫", "拿枪抢", "用枪抢", "拿刀抢", "持刀抢", "抢走", "劫财", "劫车", "抢劫得手"],
     "以武力/威胁强夺他人财产, 另需赔偿"),
    ...
    ("五·司法", "袭警罪", "重罪", 48, 25000,
     ...
     "袭击/伤害警务人员/警车/警犬"),
    ("五·司法", "违反保护令罪", "轻罪", 12, 3000,
     ["违反保护令", "违背保护令", "违反禁制令", "骚扰被保护人", "接触被保护人", "联系被保护人",
      "跟踪被保护人", "监视被保护人", "无视保护令"],
     "违反法院保护令(接触/电话短信/邮件/社交联系/监视被保护人): 轻罪1年$3000; 情节重者重罪最高3年"),
...
REG_CHARGES = [
    ...
    ("规·警务", "警员违规驾驶/警车肇事", "内务违规", 0, 0,
     ["酒后开警车", "醉驾警车", "警车违规驾驶", "下班穿工服开公车", "非任务违规驾驶警车", "警员酒驾",
      "警车撞", "警车肇事", "开警车撞", "巡逻撞", "追捕撞", "警车撞伤", "警车撞死", "驾警车撞"],
     "内务处分：严禁酒驾/违规用警车；警车撞伤撞死须上报主管+部外调查、按交通法定责（刑事另按《刑法典》：撞伤=故意袭击罪、撞死=三级谋杀罪）"),
    ("规·警务", "越权/虚报职务", "内务违规", 0, 0,
     ["冒充警衔", "谎报警衔", "虚报职务", "越权执法", "假传命令"],
     "内务调查：越权执法/虚报职务"),
    ("规·医护", "医护违法乱纪/伤害公民", "内务违规", 0, 0,
     ["医护打人", "医护殴打", "医护抢劫", "医护采毒", "医护伤人", "医护开枪", "医护参与犯罪",
      "医护打玩家", "有损医护形象", "医护违法乱纪", "医护干违法"],
     "内务处分：视情节承担，严重者直接开除；违规警告3次自动劝退（刑事按《刑法典》另算）"),
    ("规·警务", "非法搜查/错误扣留(警)", "内务违规", 0, 0,
     ["非法搜查", "无证搜查", "未经同意搜身", "非法搜身", "违规搜查", "非法扣押市民",
      "错误扣留", "乱搜身", "随意搜查市民", "无故扣留"],
     "程序违规：证据不被法庭采纳、警员受非法搜查指控；错误扣留赔偿$500/分钟"),
    ("规·警务", "公职涉黑/通匪", "内务违规", 0, 0,
     ["公职涉黑", "公职通匪", "利用公职与黑帮", "内部信息泄露", "暗中牟利", "包庇黑帮",
      "公职勾结帮派", "警匪勾结", "公职勾结黑帮"],
     "内务/服务器处分：视情节判定，严重者永久封禁（利用公职身份与黑帮合作/泄密牟利/包庇违法）"),
]

# 刑事层·补充关键词 (职务人员伤害/妨碍相关 + 车辆撞人/撞死)
EXTRA3 = {
    "妨碍政府雇员执行公务罪": ["阻挠医护", "妨碍医护", "搜查医护", "搜查医护车", "阻拦救护车",
        "阻挠急救", "拦救护车", "妨碍急救", "阻挠救援"],
    "非法制造管制物品罪": ["采集毒品", "采毒", "采集大麻", "采摘大麻", "收割大麻"],
    # 撞死(过失致死) — 含警车/各种说法
    "三级谋杀罪": ["撞死", "撞死人", "撞死市民", "撞死了", "开车撞死人", "驾车撞死",
        "警车撞死", "开警车撞死", "巡逻撞死", "追捕撞死", "把人撞死"],
    # 撞伤(车作凶器) — 含警车
    "故意袭击罪": ["警车撞人", "警车撞伤", "开警车撞人", "开警车撞", "巡逻撞人", "追捕撞人", "驾警车撞"],
}
...
REG_EXTRA = {
    ...
    "警员违规驾驶/警车肇事": ["喝酒开警车", "醉着开警车", "警车乱开", "开警车撞", "下班还穿制服开公车",
        "警员喝酒开车", "开警车肇事", "酒后驾驶警车", "警员醉驾", "飙警车", "醉驾开警车",
        "警车撞人", "警车把人撞", "巡逻车撞人", "追嫌犯撞到人", "开警车撞死", "开警车撞伤"],
    "越权/虚报职务": ["假冒长官", "冒充上级", "假传指令", "越权指挥", "虚报警衔", "谎称职务", "冒充警官身份",
        "假扮长官", "越级指挥", "谎报职务", "冒充指挥官"],
}

# =====================================================================
#  《圣安地列斯州车辆法》(交通法) —— 刑事层, 定性 违法/轻罪/重罪
# =====================================================================
TRAFFIC_CHARGES = [
    ("交通法", "无证驾驶", "轻罪", 0, 3000,
     ["无证驾驶", "没驾照开车", "无照驾驶", "没带驾照开车", "无证开车", "没驾照还开"],
     "未持有效驾照驾驶机动车（另扣押车辆）"),
    ("交通法", "持暂停/吊销驾照驾驶", "轻罪", 12, 4000,
     ["吊销驾照还开", "暂停驾照还开", "驾照吊销开车", "被吊销驾照驾驶", "无效驾照驾驶", "驾照被吊销还开车"],
     "驾照被暂停/吊销时驾驶（不构其他罪轻罪1年$4000；构其他罪如酒驾=重罪2年；多次=重罪2年扣车7天）"),
    ("交通法", "肇事逃逸罪", "违法", 0, 5000,
     ["肇事逃逸", "撞车逃逸", "撞了车就跑", "撞坏车逃", "撞车跑了", "撞坏东西逃", "刮蹭后逃", "撞车不报告"],
     "撞坏他人财物未及时报告（另扣车+赔偿）；致人受伤见严重肇事逃逸罪"),
    ("交通法", "严重肇事逃逸罪", "重罪", 36, 10000,
     ["严重肇事逃逸", "撞伤人逃逸", "撞人逃逸", "撞伤人就跑", "撞人跑了", "撞人后逃", "撞伤后逃逸",
      "撞伤人后逃", "撞人后逃逸", "撞了人逃", "撞伤人后逃逸", "撞人致伤逃逸", "撞伤逃逸"],
     "撞致他人受伤未及时报告（另赔偿）"),
    ("交通法", "右侧驾驶/越线违规", "违法", 0, 4000,
     ["逆行", "逆向行驶", "越实线", "压双黄线", "压实线", "不靠右行驶", "越线行驶", "越黄线", "压线行驶"],
     "未靠右/超车越黄白实线/越隔离带行驶"),
    ("交通法", "不避让应急车辆", "违法", 0, 8000,
     ["不避让救护车", "不避让警车", "不让应急车辆", "不给救护车让路", "挡应急车道",
      "不避让消防车", "不给警车让路", "堵住应急车辆"],
     "遇警报声/红蓝灯应急车辆未立即靠右让行"),
    ("交通法", "超速", "违法", 0, 2000,
     ["超速", "超过限速", "开太快超速", "严重超速", "飙速行驶"],
     "超速10MPH内/低于最低限速/非法停车≤$500；超10-20=$500-1000；超20以上=$2000"),
    ("交通法", "违规停车", "违法", 0, 500,
     ["违规停车", "乱停车", "违章停车", "占道停车", "堵车道停车", "乱停乱放"],
     "在禁停处/妨碍交通处停车"),
    ("交通法", "危险驾驶", "轻罪", 12, 5000,
     ["危险驾驶", "横冲直撞", "乱开车", "多次撞击", "鲁莽驾驶", "无视安全驾驶", "开车横冲", "莽夫开车"],
     "漠视安全(逆行/多次撞击/破坏公共财产)/超速+危险因素/违规威胁公共安全（另扣车）"),
    ("交通法", "飙车", "轻罪", 24, 5000,
     ["飙车", "赛车", "竞速", "街头飙车", "计时竞速", "组织飙车", "公路竞速", "非法竞速"],
     "公共道路两车以上计时竞速 / 帮助竞速或设障（另暂停驾照）"),
    ("交通法", "醉驾或毒驾", "轻罪", 24, 8000,
     ["醉驾", "酒驾", "酒后驾驶", "毒驾", "吸毒驾驶", "喝酒开车", "酒后开车", "醉酒驾驶", "嗑药开车"],
     "BAC≥0.08%或服管制物品驾驶（另扣车+暂停所有驾驶许可）"),
    ("交通法", "严重醉驾或毒驾", "重罪", 36, 10000,
     ["严重醉驾", "严重酒驾", "严重毒驾", "醉驾撞人", "酒驾肇事", "酒驾撞", "醉驾肇事", "多次酒驾"],
     "BAC≥0.15%且危害他人/一年内4次以上违规（另扣车+吊销驾照）"),
    ("交通法", "非法车辆改装", "违法", 0, 15000,
     ["非法改装", "改装车", "氮气改装", "赛用改装", "车窗贴膜过深", "改装套件", "非法改装车辆",
      "加装氮气", "暴力改装"],
     "氮气/赛用改装/车窗透光率<70%/私改未报备（另拆除非法改装件）"),
]

# =====================================================================
#  《涉及商业经营的违法行为》(经营法) —— 刑事层, 定性 违法
# =====================================================================
BUSINESS_CHARGES = [
    ("经营法", "售卖非注册商品", "违法", 0, 10000,
     ["售卖非注册商品", "卖非注册商品", "卖违规商品", "出售非注册", "卖非法商品", "贩卖非注册"],
     "向他人售卖盈利场所注册商品以外的商品"),
    ("经营法", "虚假报价", "违法", 0, 20000,
     ["虚假报价", "虚报价格", "乱报价", "谎报价格", "虚报商品价格", "报价造假"],
     "向他人虚报商品价格"),
    ("经营法", "非法经营", "违法", 0, 8000,
     ["非法经营", "无证经营", "无营业许可", "没营业执照经营", "黑店经营", "未申请经营", "无照经营"],
     "未申请/无营业许可进行盈利活动（另没收贩卖商品）"),
    ("经营法", "拒绝食品/监督检查", "违法", 0, 20000,
     ["拒绝食品检查", "拒绝监督检查", "阻挠食品安全检查", "拒绝营业检查", "干涉监督检查",
      "拒绝风险评估", "阻挠事故调查"],
     "拒绝/阻挠政府食品安全监督检查等（另停产营业；严重者取消许可/没收保证金/冻结盈利）"),
    ("经营法", "拒绝缴纳税款", "违法", 0, 0,
     ["拒绝缴税", "偷税", "逃税", "拒缴税款", "推脱缴税", "不交税", "拒不纳税", "偷税漏税"],
     "盈利场所拒绝/推脱缴税（取消营业许可，没收保证金及营业公款）"),
    ("经营法", "虚假财务报表", "违法", 0, 30000,
     ["虚假财务报表", "做假账", "假财报", "隐瞒经营情况", "虚报收入", "财务造假", "假账目"],
     "向政府隐瞒/提供虚假经营情况、人员、收入（另罚没保证金、吊销营业执照及经营公款）"),
]

# =====================================================================
#  服务器 / RP 违规 (OOC) —— 定性 服务器违规, 处罚=监狱扮演时长/封禁, 非刑期
#  来源: 服务器违规处罚表、黑吃黑规则
# =====================================================================
SERVER_CHARGES = [
    ("服·驾驶", "随意撞人/恶意车杀(VDM)", "服务器违规", 0, 0,
     ["VDM", "随意撞人", "恶意车杀", "故意用车撞", "车辆攻击", "开车撞人", "驾车撞人", "用车撞人",
      "故意撞车", "车杀", "开车撞玩家", "恶意撞人"],
     "处罚：3h-6h 监狱扮演（IC 另按撞伤=故意袭击/撞死=三级谋杀）"),
    ("服·驾驶", "违规PIT", "服务器违规", 0, 0,
     ["违规PIT", "乱PIT", "未警告PIT", "PIT违规", "没警告就PIT"],
     "处罚：警车须警告三次无果方可 PIT，成功三次后须停车"),
    ("服·战斗", "随意击杀(DM)", "服务器违规", 0, 0,
     ["DM", "随意击杀", "无理由击杀", "无RP击杀", "乱杀人", "无故击杀", "秒杀玩家", "上来就杀", "无RP链接击杀"],
     "处罚：3h-6h 监狱扮演（无理由/无 RP 直接击杀）"),
    ("服·战斗", "安全区击杀", "服务器违规", 0, 0,
     ["安全区击杀", "医院击杀", "警局击杀", "监狱击杀", "新人区击杀", "市政击杀", "安全区开枪",
      "安全区杀人", "安全区动手", "新手区击杀", "医院里击杀", "在医院击杀", "在警局杀", "安全区内击杀",
      "新人区杀人", "医院开枪", "警局开枪"],
     "处罚：3h-6h 监狱扮演（医院/警局/监狱/新人区/市政等安全区禁致命攻击）"),
    ("服·战斗", "不惧怕热武器", "服务器违规", 0, 0,
     ["不惧怕热武器", "不惧热武器", "不怕热武器", "不怕枪", "不惧枪", "无视热武器",
      "面对枪口反抗", "无视枪口", "被指枪还反抗", "被枪指还反抗", "被枪指着还反抗",
      "毫无恐惧反抗", "无视威胁反抗", "顶着枪反抗", "顶着枪口反抗", "被指枪不怕",
      "枪口下不怕", "枪指着不怕", "无惧枪口"],
     "处罚：3h-6h 监狱扮演（面对明确枪口威胁须有恐惧感、不得不合理反抗）"),
    ("服·战斗", "买凶违规", "服务器违规", 0, 0,
     ["买凶违规", "未提示击杀", "没提示就杀", "买凶未提示"],
     "处罚：3h-6h 监狱扮演（未给目标明确提示直接击杀）"),
    ("服·抢劫", "违规抢劫/摸包", "服务器违规", 0, 0,
     ["违规抢劫", "违规摸包", "抢劫新人", "抢新人", "对无关玩家搜身", "违反抢劫间隔", "乱摸包",
      "抢劫间隔不足", "重复抢劫", "组织间联合抢劫", "联合抢劫", "抢安全区玩家", "对无关尸体搜身"],
     "处罚：2h-3h 监狱扮演（间隔30分/同目标3小时/抢新人或安全区/联合抢劫/搜无关玩家）"),
    ("服·帮派", "帮派行为违规", "服务器违规", 0, 0,
     ["帮派违规", "合作帮派攻击", "非法联合", "帮派非法联合", "保护费违规", "驱逐侵者不符RP", "帮派联合攻击"],
     "处罚：2h-3h 监狱扮演（合作帮派攻击/非法联合/行为不符 RP 逻辑）"),
    ("服·帮派", "黑吃黑违规", "服务器违规", 0, 0,
     ["黑吃黑违规", "黑吃黑插队", "未报备黑吃黑", "提前介入黑吃黑", "黑吃黑提前开火", "干扰原抢劫",
      "黑吃黑超编", "黑吃黑未驱离开火"],
     "处罚：按黑吃黑规则与管理判定（须报备工单/按顺位/不得提前开火/驱离须互动/不得干扰原抢劫）"),
    ("服·扮演", "复仇违规", "服务器违规", 0, 0,
     ["复仇违规", "死后寻仇", "死亡后立即寻仇", "死后立即寻仇", "立即报复", "刚复活就报复", "回血就寻仇",
      "复仇缺RP", "复仇没RP", "冷却不足寻仇", "死后继续寻仇", "死后立刻报复", "刚死就报复"],
     "处罚：1h-6h 监狱扮演（死亡后/冷却不足20分/缺合理 RP/未报 rp 工单）"),
    ("服·扮演", "混淆扮演(IC/OOC)", "服务器违规", 0, 0,
     ["混淆扮演", "IC/OOC混淆", "ICOOC混淆", "用现实信息", "现实与游戏混", "出戏", "元游戏",
      "metagaming", "IC提OOC", "利用IC信息泄露OOC", "在IC提及OOC"],
     "处罚：警告→2h 监狱扮演（把现实与游戏混为一谈/用现实信息影响 RP）"),
    ("服·扮演", "不合理/低端扮演", "服务器违规", 0, 0,
     ["不合理扮演", "无视威胁", "超出角色能力", "违反现实常识", "破坏RP", "车辆翻滚仍完美驾驶",
      "爆胎仍完美驾驶", "强行推动剧情", "无视他人角色反应", "消极RP", "摆烂演出", "敷衍演出",
      "轻视医疗事件", "阴阳怪气"],
     "处罚：3h-6h 监狱扮演（消极 RP/超出角色能力/违反现实常识/破坏他人 RP）"),
    ("服·行为", "行为类违规(辱骂/滋事)", "服务器违规", 0, 0,
     ["辱骂玩家", "OOC辱骂", "挑衅引战", "恶意滋事", "扰乱管理", "顶撞管理员", "侮辱女玩家",
      "性别侮辱", "持续骚扰", "言语羞辱"],
     "处罚：2h 监狱扮演或永久封禁（OOC 辱骂/引战/扰乱管理/侮辱骚扰女玩家）"),
    ("服·技术", "技术类违规(开挂/卡BUG)", "服务器违规", 0, 0,
     ["利用BUG", "卡点", "刷物", "刷钱", "异常获利", "开挂", "外挂", "第三方插件", "修改器",
      "脚本作弊", "恶意卡麦", "卡掉线", "卡顿逃避", "利用漏洞"],
     "处罚：视情节，轻微 CK（角色死亡/警告），严重永久封禁"),
    ("服·封禁", "私下交易/场外交易", "服务器违规", 0, 0,
     ["私下交易", "私群交易", "拉进私群", "拉新市民到私群", "非官方交易", "场外交易", "私自交易道具",
      "买卖账号", "现实货币买卖", "RMT", "卖号"],
     "处罚：查到清档；主动购买/参与一次即永久封禁"),
]

# 合并去重（刑法典 + 交通法 + 经营法 + 内务违规 + 服务器违规）
NON_CRIMINAL_KINDS = ("内务违规", "服务器违规")
_merged = []
for chap, name, kind, months, fine, kws, note in (
        CHARGES + TRAFFIC_CHARGES + BUSINESS_CHARGES + REG_CHARGES + SERVER_CHARGES):
    allk = list(dict.fromkeys(
        kws + EXTRA_KEYWORDS.get(name, []) + EXTRA_KEYWORDS2.get(name, [])
        + REG_EXTRA.get(name, []) + EXTRA3.get(name, [])))
...
def build_law_and_kw(ws_law, ws_kw):
    ...
    for i, (chap, name, kind, months, fine, kws, note) in enumerate(CHARGES):
        ...
        # 刑事 = 非(内务违规/服务器违规); 违规 = 内务违规 或 服务器违规
        cC = lambda col: (f'AND(${col}{r}=1,$C{r}<>"内务违规",$C{r}<>"服务器违规")')   # 刑事命中
        vC = lambda col: (f'AND(${col}{r}=1,OR($C{r}="内务违规",$C{r}="服务器违规"))')   # 违规命中
        if r == law_first:
            ws_law.cell(row=r, column=10, value=f'=IF({cC("G")},$B{r},"")')
            ws_law.cell(row=r, column=11, value=f'=IF({cC("H")},$B{r},"")')
            ws_law.cell(row=r, column=12, value=f'=IF({cC("I")},$B{r},"")')
            ws_law.cell(row=r, column=13, value=f'=IF({vC("G")},$B{r}&"（"&$F{r}&"）","")')
            ws_law.cell(row=r, column=14, value=f'=IF({vC("H")},$B{r}&"（"&$F{r}&"）","")')
            ws_law.cell(row=r, column=15, value=f'=IF({vC("I")},$B{r}&"（"&$F{r}&"）","")')
        else:
            ws_law.cell(row=r, column=10, value=f'=IF({cC("G")},IF($J{r-1}="",$B{r},$J{r-1}&"、"&$B{r}),$J{r-1})')
            ws_law.cell(row=r, column=11, value=f'=IF({cC("H")},IF($K{r-1}="",$B{r},$K{r-1}&"、"&$B{r}),$K{r-1})')
            ws_law.cell(row=r, column=12, value=f'=IF({cC("I")},IF($L{r-1}="",$B{r},$L{r-1}&"、"&$B{r}),$L{r-1})')
            ws_law.cell(row=r, column=13, value=f'=IF({vC("G")},IF($M{r-1}="",$B{r}&"（"&$F{r}&"）",$M{r-1}&"；"&$B{r}&"（"&$F{r}&"）"),$M{r-1})')
            ws_law.cell(row=r, column=14, value=f'=IF({vC("H")},IF($N{r-1}="",$B{r}&"（"&$F{r}&"）",$N{r-1}&"；"&$B{r}&"（"&$F{r}&"）"),$N{r-1})')
            ws_law.cell(row=r, column=15, value=f'=IF({vC("I")},IF($O{r-1}="",$B{r}&"（"&$F{r}&"）",$O{r-1}&"；"&$B{r}&"（"&$F{r}&"）"),$O{r-1})')
...
def build_main(m):
    ...
    s3.value = "　③ 违规 / 处分　—　内务处分(警员/医护) + 服务器RP违规(DM/VDM/安全区/混淆扮演等)；刑事罪名已在上方照常计入"
...
RULES = [
    ...
    ("H", "🚑 医护 · 伤害公民相关", ""),
    ("R", "意外撞伤/撞死玩家", "非任务、未开警灯警笛时撞伤/撞死玩家：免收一切费用，并须优先救治被撞者。"),
    ("R", "违法乱纪伤人", "医护采毒/抢劫/殴打玩家等：视情节，严重者直接开除；违规累计警告 3 次自动劝退。"),
    ("R", "妨碍医护(对市民)", "阻挠医护执行任务、搜查医护车辆/人员：构成「妨碍政府雇员执行公务罪」（已在算罪台刑事层）。"),
    ("H", "⚖ 警务 · 执法过当与赔偿", ""),
    ("R", "武力致伤亡", "警械使用导致伤亡：须立即处置伤者、呼叫巡逻部门主管到场，必要时呼叫部外调查单位；错误使用致命武器受内务组调查。"),
    ("R", "非法搜查", "未经同意/无搜查令的搜查＝非法搜查：证据不被法庭采纳，警员可能受「非法搜查」指控。"),
    ("R", "错误扣留赔偿", "警局错误扣留造成的赔偿为 1 分钟（国际时间）＝ $500。"),
    ("R", "警车撞伤/撞死公民", "刑事按《刑法典》：撞伤=故意袭击罪、撞死=三级谋杀罪（与平民一视同仁）。内务：警车肇事/违规驾驶须上报监督员、呼叫高一级警员、按交通法定责，必要时部外调查；任务期间也须确保无人受伤、严禁酒驾。"),
    ("R", "虚假逮捕/非法没收", "无依据以执法名义扣留/逮捕/没收财物：构成「虚假逮捕罪」（重罪，已在算罪台刑事层，仅部门主管可起诉）。"),
    ("H", "❗ 总则 · 职务人员伤害公民", ""),
    ("R", "两层并罚", "职务人员（警员/医护）伤害公民时：① 刑事按《圣安地列斯州刑法典》照常追责（与平民一视同仁，如袭击/谋杀/虚假逮捕）；② 另叠加内务处分（调查/停职/降级/开除/FBI）。两者并行，不因身份免刑。"),
    ("H", "🎮 服务器 · RP 违规处罚（OOC）", ""),
    ("R", "战斗/驾驶类", "随意击杀DM、随意撞人VDM、安全区击杀、复仇违规、买凶违规、不惧热武器、违规抢劫摸包：3h-6h 监狱扮演（撞人/杀人 IC 另按刑法典）。"),
    ("R", "抢劫/帮派类", "抢劫未间隔30分/同目标3小时/抢新人或安全区/联合抢劫、帮派非法联合：2h-3h 监狱扮演。"),
    ("R", "扮演类", "混淆扮演(IC/OOC)：警告→2h；不合理/低端扮演(消极RP/超能力/违反现实常识)：3h-6h。"),
    ("R", "技术/封禁类", "开挂/卡BUG/刷物/修改器/恶意卡麦：轻微 CK、严重永 ban；私下交易/现实货币买卖/拉私群：清档，主动参与一次即永久封禁。"),
    ("R", "公职涉黑", "利用公职身份与黑帮合作、泄密牟利、包庇违法：视情节，严重者永久封禁。"),
    ("H", "🃏 黑吃黑 · 规则要点", ""),
    ("R", "报备与顺位", "全程录屏；行动前提交工单报备（标题写明目标，如「黑吃黑-太平洋银行」）；管理按提交顺序记 OBS1/OBS2…，严格按顺位进行，不得插队。"),
    ("R", "参与与人数", "统一穿帮派服；被黑方人数「透明」时黑吃黑人数须≤合理推测人数，严禁超编；现场范围=目标点+周边两街区，停留超30秒视为进入现场；提供信息/观察/报点也算参与。"),
